...
import { ChevronDown, History, User, LogOut } from "lucide-react";
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen((o) => !o)} className="flex items-center gap-2.5 group">
        <div className="w-9 h-9 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs font-['DM_Mono'] font-medium group-hover:bg-accent transition-colors duration-200">
          {initials}
        </div>
        ...
        <div className="absolute right-0 top-full mt-3 w-64 bg-card border border-border rounded-2xl shadow-lg overflow-hidden z-50" style={{ animation: "fadeSlideUp 0.2s ease-out" }}>
          ...
          <div className="px-4 py-4 border-b border-border bg-secondary/30">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-sm font-['DM_Mono'] font-medium shrink-0">
                {initials}
              </div>
